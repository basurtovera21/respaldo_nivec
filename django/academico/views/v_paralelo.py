...
def _paralelos_del_grupo(representativo):
    # Un paralelo lógico = filas (una por unidad) que comparten periodo, carrera, jornada y nombre.
...
def listar_paralelos(request):
    ...
    # Agrupamos las filas (una por unidad) en paralelos lógicos.
    grupos = {}
    for p in paralelos:
...
def eliminar_paralelo(request, paralelo_id):
    ...
    # Se elimina el paralelo lógico completo (todas sus unidades) y sus matrículas.
    ...
    messages.success(request, f"El {nombre} ha sido eliminado correctamente")
    return redirect("listar_paralelos")
